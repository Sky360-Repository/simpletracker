...
class VideoTracker():
    ...
    def process_frame(self, frame, frame_count, fps):
        ...
        frame = utils.resize_frame(
            self.resize_frame, frame, self.enable_cuda, self.normalised_w_h[0], self.normalised_w_h[1])
        ...
        frame_gray = utils.noise_reduction(
            self.noise_reduction, frame_gray, self.blur_radius)
        ...
        if self.detection_mode == 'background_subtraction':
            ...
            if self.calculate_optical_flow:
                ...
                optical_flow_thread = Thread(
                    target=self.perform_optical_flow_task, args=(frame_count, frame_gray, tic7))
